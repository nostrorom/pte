...
export const _buffer = await AUDIO.decodeAudioData(await (await fetch('/ヨ.wav')).arrayBuffer());
...
export default ((channel?: Channel, duration = 1, volume = 100) => {
	...
	return { createSource, volume, duration, channel, type: 'sound' as const, name: 'ヨ' };
}) satisfies Note;
